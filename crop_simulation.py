...
class CropWindow(QMainWindow):
    ...
    def create_view_crop_layout(self,crop_type):
        ...
        self.crop_view.setHorizentalScrollBarPolicy(1)
    ...
    def instantiate_crop(self):
        ...
        self.create_view_crop_layout(crop_type) #create the view crop growth view layout
    ...
    def update_crop_view_status(self):
        ...
        if crop_status_report["status"] == "Seedling":
            self.crop_view.switch_scene(1)
        if crop_status_report["status"] == "Young":
            self.crop_view.switch_scene(2)
        if crop_status_report["status"] == "Mature":
            self.crop_view.switch_scene(3)
        if crop_status_report["status"] == "Old":
